...
**以游戏还原人类加密解密的三千年历史**——一座「密码博物馆」：首页破译旅程画卷（12 时代时间轴）+ 编年史 + 人物志 + 密件册 + 游戏厅 + 我的档案，全局导航统一。

内置 **114 款小游戏**，街机霓虹暗黑风，最高纪录保存在浏览器 localStorage。
...
| 触感反馈 | `navigator.vibrate` 随音效联动（快捷栏 🫨 开关，仅触屏设备显示） |
 | 主题色联动 | 双主题（街机霓虹 / 晨光档案）切换时同步 `theme-color` meta 与 iOS 状态栏样式；默认街机，不跟随系统（2026-08 收敛） |
| 安全区 | 全站 viewport 含 `viewport-fit=cover`（133 个 HTML 由工具批量同步） |
...
- **首页**（`index.html`）：破译旅程画卷——12 时代时间轴，每节点聚合故事/人物/密件/游戏；破译进度总览 + 继续破译（最近游玩）
- **游戏厅**（`games.html`）：114 款游戏，搜索 / 分类筛选 / 收藏 / 每日破译 / 最近游玩，游戏卡带「📜 史话 xN」编年史徽章
- **编年史**（`stories.html`）：12 章真实密码学史时间轴
- **人物志**（`people.html`）：48 位破译者档案卡
- **密件册**（`artifacts.html`）：通关游戏解锁 41 件历史密件
...
| 分类 | 游戏 |
|---|---|
| 密码破译 (48) | 猜词破译 · 凯撒解码 · 摩斯破译 · 大师密码 · 替换密码 · 维吉尼亚 · 摩斯长报文 · 二进制破译 · 打字破译 · 栅栏密码 · 仿射密码 · Base64 破译 · 摩斯听写 · 词频分析 · 恩尼格玛 · Playfair · 异或破译 · 破译战役 · ADFGVX · 密码侦探 · Bifid · 炸弹机 · 希尔密码机 · 破译工作室 · 密码地牢 · VENONA 双密复用 · JN-25 破译机 · 插线板反推 · Trifid 破译机 · 紫密破译机 · M-209 转轮密码机 · 洛伦兹破译机 · 密码制造者 · 找茬破译 · 培根密码机 · 阿特巴什 · 波利比奥斯方阵 · 尼希尔斯特 · 星条旗密码 |
...
├── stats.html               # 我的档案页
├── sitemap.xml              # 站点地图（131 URL，从注册表生成）
├── robots.txt               # 搜索引擎收录配置
└── games/<游戏id>/
...
node smoke.js                 # 全站 114 款冒烟（DOM/画布/音频加载 + 初始化 + 重开 + 一帧渲染）
node smoke.js zh|en           # 双语模式冒烟（占位符/缺键/渲染污染检测）
node smoke.js page [en]       # 站点页面冒烟（18 个页面）
...
## 🚀 第六期批次二 + 收官：Shamir 分钥 + CTR/位错扩散 + LSB 隐写工坊（2026-08-24 已实施）

> 第六期至此**全部收官**；终态回归 qa-all **21/21 PASS** · play-probe+games-all **222/222** · 批次二专项 e2e **5/5**。

| # | 内容 | 结果 |
|---|---|---|
| #1 | 第 **111** 款游戏 `shamir`「Shamir 分钥密约」：k=2 门限秘密分享五步手算链——分发验证 f(x)=s+a·x mod 101 → 门限认知三选一 → 收集份数判断（1/2/4 份轮换）→ 相邻份额相减求斜率 → 回归 x=0 还原秘密字母；3 轮 ×（首答+20+连击、整轮+30、提示−10）；daily 日种子出题 | 旗舰标准全过 |
| #3 | 模式实验室二期：① CTR 计数器模式入列（ECB/CBC/CTR 三图并列，密钥流=加密计数器逐字节异或）；② 「⚡ 位错误扩散」对照演示——同一错位密文分别解密，CBC 整块花掉+下块单位翻转自愈 vs CTR 精确到比特只损一像素；玩具密码补 S⁻¹ 逆置换支持真解密 | 经典工程取舍可视化 |
| #2 | 工坊第四 Tab「🖼️ 隐写工坊」：LSB 图像隐写——文本 UTF-8 编码 + 32 位大端长度头写入 R/G/B 最低有效位（不碰 alpha）；程序化黄昏海湾示例图（免上传即可玩可测）+ 图片上传适配；容量实时指示（192×192×3 位）；提取带长度头校验防误报；💾 PNG 导出；「🔬 位平面查看器」（bit7 主旋律 vs bit0 噪点纹理）落地隐写分析直觉 | 全网独家加密/破解/隐写三合一实验室 |
| 集成 | 注册表/BEST_UNITS(Pts)/DAILY_IDS(21 款)/g.shamir 键/st.c10.g17 末位追加/sitemap 127 URL/SW 缓存 v19；workshop 页签切换重构为 TABS 数组统一驱动（顺修 modes 标签缺 i18n 键） | 全绿 |

**开发中揪出的真 bug**：隐写长度头首版只写 28 位却按 32 位读取，整体错位 ×16 导致提取尾部乱码——真浏览器最小复现脚本逐位对拍定位后改为标准 32 位大端写入。

验证器沉淀：`e2e/r6-batch2.spec.js`（shamir 完整通关 / CTR 与错误扩散像素级断言：CTR 受损 ≤1 像素且 CBC 受损面严格更大 / 隐写往返一致 + alpha 不动 + 改动像素占比 <20% / modern 章 shamir 文案双语渲染）。

## 🚀 第七期：协议实验室 protocols.html（2026-08-24 已实施）

> 按 `docs/coverage-roadmap.md` 第七期规划一次交付全部 6 项；终态回归 qa-all **21/21 PASS**（页面冒烟扩容 **18/18**）· 专项 e2e `e2e/r7.spec.js` **6/6**。

| # | 内容 | 结果 |
|---|---|---|
| 7 | 🤝 TLS 握手时序：六步逐帧推进，标注每步明文/密文边界；「中间人在场」开关揭示 Eve 在证书验证步露馅的机制 | 防 MITM 根基可视化 |
| 8 | 🕵️ DH 中间人攻防：p=23,g=5 玩具参数完整数值演算；Eve 开关下双方各与 Eve 协商出**两把不相等密钥**而协议无察觉——与 BB84「窃听留痕」形成课程级对照，解药指向第 7 项的数字证书 | 全站知识闭环 |
| 9 | 🌳 Merkle 树 + 区块链：点击改任意交易 → 哈希向上传染至根（flash 动画）；篡改第 2 块 → 本块哈希变红 + 第 3 块 prev 断链，「不可篡改」变成看得见的机械过程 | djb2 玩具哈希 |
| 10 | 🎭 零知识证明：三色图承诺-挑战-应答全流程——每轮随机换色名承诺 → 随机挑边 → 仅揭两端（永远异色），节点模糊动画 + 轮次统计「每轮泄露 ≈0」 | 经典 ZKP 教学原型 |
| 11 | 📈 ECC 点加法：实数域 y²=x³+ax+b 曲线随滑杆实时重绘（奇异曲线守卫）；点选 P、Q → 割线/切线几何作图出第三交点镜像；含无穷远点提示 | 离散对数难题几何化 |
| 12 | ⏳ 口令破解成本计算器：字符集×长度→组合数/熵；4 档算法（MD5⚠/SHA-256/bcrypt/Argon2id）×4 档装备（CPU→百万卡）实时换算穷举时间并分级着色——「长度就是一切」「Argon2 打回百万卡」两条直觉落地 | 接入 Argon2 词条叙事 |
| 集成 | 新页 `protocols.html`（128 URL）+ `assets/js/protocols.js` 双语数据驱动；smoke 页面清单 17→18、sitemap/prefetch/SW 预缓存 v20 同步；工坊 hero 加入口链接；`#pl-ready` 冒烟标记 | 门禁全绿 |

开发中修复的真 bug：DH 的 Eve 开关误绑 `change` 事件（按钮不触发）；Merkle 叶子改动正则匹配不到非十六进制尾字符导致哈希不变。ECC 选点像素微偏属正常采样粒度，测试改为从 `data-last` 读回实际选中点按同式推期望（曲线残差 <0.05 断言）。

## 🏁 第八期：深水区全部清零（2026-08-24 已实施）

> 路线图最后一期收官；终态回归 qa-all **21/21** · play-probe+games-all **228/228** · 专项 e2e `r8.spec.js`（含 r7 回归共 11/11）。

| # | 内容 | 结果 |
|---|---|---|
| 13 | 第 **112** 款游戏 `sm4`「SM4 国密试炼场」：**真实 SM4 引擎**——S 盒/FK/CK 全部取自 IETF draft-ribose-cfrg-sm4-10（CK 按标准公式 (4i+j)×7 mod 256 现场生成），启动即对拍 GB/T 32907 附录示例向量 `681EDF34…4246` 并显示徽章；玩法=S 盒查表（附官方表展开）+ 官方轮迹实时计算追踪 + 结构/史话问答 | 国密真实现落地 |
| 17 | 第 **113** 款游戏 `acrostic`「藏头诗密信」：中文行首字/英文行首字母连读解谜，原创中英双语诗库（8 中文 +5 英文），答对高亮首字验证；挂靠 c3 培根章（与双字体隐写同源） | 中文站特色差异化 |
| 18 | 第 **114** 款游戏 `phishhunt`「钓鱼邮件狩猎」：真假邮件八连判，9 个教学虚构案例覆盖仿冒域名/回复错位/礼品卡诈骗/IP 裸地址/反向收费等经典套路，判后线索自动亮出；挂靠 c10 现代章 | 社会工程面补齐 |
| 14 | protocols 新增 🌀 ChaCha20 卡：4×4 状态实时单步——列 QR→对角 QR 共 64 操作逐条执行，操作数高亮 + 公式同步 | 流密码现代标杆可视化 |
| 16 | protocols 新增 📡 A5/1 卡：19/22/23 三 LFSR 多数投票钟控逐步演示，时钟位/抽头位高亮、密钥流计数；Nohl 2009 彩虹表攻破史入注 | 历史警示教育 |
| 15 | machine.html 新增第 6 台机器「AES Rijndael」卡 + 四拍动画实验室：SubBytes→ShiftRows→MixColumns→AddRoundKey 逐步/自动播放，受影响格子发光（示意运算并明确标注非真实 S 盒） | 现代分组密码结构动画 |
| 集成 | sitemap 131 URL · SW 缓存 v21 · DAILY_IDS 24 款 · smoke 页面断言 pl-ready 6→8 · 键前缀规范化 gs.ph.→gs.phishhunt.（消除校验器盲区） | 门禁全绿 |

开发中修复的真 bug：SM4 自检把字级 tau 当字节 S 用（S(EF) 应查单字节表）；protocols 的 isEn 在子作用域误作函数调用。SM4 引擎另经独立 Node 对拍：加密向量 ✓、rk0=F12186F9 ✓、加解密往返 ✓。

## 🎨 主题收敛：街机 + 晨光双档（2026-08-24 已实施）

> 应用设置收敛为双主题：**街机霓虹（默认）** 与 **晨光档案**；移除 auto 跟随系统档与其余四套配色。历史存档（auto/已下线主题）自动迁移回街机并落盘；晨光用户保留。快捷栏 🎨 按钮循环切换，theme-color/状态栏联动同步收敛。theme.css 移除四套死配色（-87 行），i18n 清理 5 对死键。回归：qa-all 21/21 · chromium 全量 **434/434**（pwa.spec 主题用例重写为双档语义）。

## ✨ 内容打磨轮：联动补链与复玩性扩容（2026-08-24 已实施）

> 红区三件套 + 中优先级池一次交付；终态回归 qa-all **21/21** · r7+r8 e2e **11/11**。

| 组 | 内容 | 结果 |
|---|---|---|
| 互链 | 术语表 × 新游戏补链：RSA→rsa · Shamir 门限→shamir · SM4→sm4 · 隐写术→acrostic；**新增「钓鱼攻击」词条**→phishhunt | 知识闭环修复 |
| 题库 | 测验 110→**120 题**（新增 SM4×2 / Shamir×2 / TLS1.3 / 隐写 / 反钓鱼×2 / ChaCha20 / A5-1 攻破史；分布 21/33/36/30，draw10 恒定校验通过）；全站计数 110→120 六处同步（dict/ui/quiz.html meta+正文） | 新知识点全部入考 |
| 诚实性 | protocols TLS 卡补 **TLS 1.3 注脚**：强制 (EC)DHE、废弃 RSA 传输、握手后全加密 | 消除时代错位 |
| 复玩性 | phishhunt 案例 9→**15**（OAuth 授权钓鱼/快递双倍理赔/刷单垫付 + 内部 Wiki/航司确认/GitHub 公告）；acrostic 英诗 5→**8**（SHADOW/ORACLE/PUZZLE）；sm4 结构题 6→**12**、史话题 4→**8**（轮数对比/tau-L 分工/结构归属/R 变换/ISO 收录/国密分工/公开承诺） | 三款新游戏耐玩度翻倍 |
| 路径 | path.html 新增 CURATED_REST 机制：c3 第 2 天编入 acrostic+autokey、c10 第 2 天编入 sm4+phishhunt | 学习路径跟上内容 |
| 博物馆 | machine 新增第 7 台「💾 DES」：56 位王座的二十年统治与 EFF Deep Crack 22.5h 覆灭史；game 链接条件化渲染 | 历史叙事补完 |
| 实验室 | protocols 第 9 卡 🧨 RC4 警示录：真实 RC4(KSA+PRGA) 复现 WEP 式 IV 不变灾难——窃听者无钥即得 P1⊕P2=C1⊕C2 | RFC 7465 禁用史可视化 |

## ✍️ 第九期：数字签名 + CSPRNG（2026-08-24 立项即实施）

> 概念面最后两块拼图；终态回归 qa-all **22/22** · chromium 全量 **436/436** · r7 扩容 **8/8**。

| # | 内容 | 结果 |
|---|---|---|
| 19 | protocols 第 10 卡「✍️ 数字签名」：小参数 RSA（p=5,q=11,e=3,d=27）走 私钥签名(s=m^d)→Eve 篡改→公钥验证(s^e≠m')✗ 识破 全流程；滑杆换消息可复验 ✓；含「公钥锁/私钥开 vs 私钥锁/公钥开」方向对照与真实系统哈希+PSS 注脚 | 公钥体系另一半补齐 |
| 20 | protocols 第 11 卡「🎲 随机数」：时间种子 LCG 生成"随机"密钥 → 攻击者枚举 86400 秒种子毫秒级命中 → 整条密钥流可预测；明示 Math.random(V8 xorshift128+) 同理，密码学必须 CSPRNG/OS 熵源 | 地基级缺口补齐 |
| 集成 | pl-ready 9→11（smoke/r7/r8 断言同步）· meta/og/ld+json 十一大交互演示 · SW v24 · 覆盖图 §1.3 签名/CSPRNG 🔲→✅ | 概念面 ≈80% |
...
- [x] 国密游戏化：SM4 真实现 + 测试向量对拍（第八期 sm4，2026-08-24）
