...
Arcade.i18n.dicts = {
  zh: {
    ...
    'g.fanqie.t': '反切码军情室', 'g.fanqie.d': '戚继光《纪效新书》的两字暗号：上字定声、下字定韵，十六声母 × 十六韵母查表传军情。',
    'g.otp-telegraph.t': 'OTP 电报房', 'g.otp-telegraph.d': 'Vernam 1917 电报机真机体验：ITA2 五单位码与密钥带逐位异或——再把两封同密钥电文相减，看见深度。',
    'g.tor-onion.t': 'Tor 洋葱路由', 'g.tor-onion.d': '按真实规则亲手建一条三跳电路：逐层封装与剥皮、出口节点可见性、流量分析——匿名网络的每一层皮。',
    'g.block-modes.t': '块模式实验室', 'g.block-modes.d': '同一块密码、模式决定命运：ECB 泄漏图案、CBC 链接与 IV 铁律、CTR 计数器流、GCM 一举两得。',
    'g.dh-handshake.t': 'DH 握手场', 'g.dh-handshake.d': '1976 新方向的手算现场：真实模幂交换、共享密钥验证、Eve 中间人插入、参数退化陷阱（g=1 时密钥恒为 1）。',
    'g.gm-gateway.t': '国密网关', 'g.gm-gateway.d': '一条报文走完国密管线：SM4 真引擎加密、SM3 真实现摘要、微型曲线 SM2 式验签、SM9 标识密码。',
    'g.millionaire.t': '百万富翁协议', 'g.millionaire.d': '姚氏 1982 开山问题：只比谁更富、谁也不露数——玩具 RSA 盲化、1-out-of-2 OT、决策树首次分歧与 MPC 版图。',
    'g.blind-sign.t': '盲签密约', 'g.blind-sign.d': 'Chaum 1983 电子现金数学真机：RSA 盲化、银行对看不见的文件签名、去盲得普通签名、盲性判断与双花检测。',
  ...
  en: {
    ...
    'quiz.entrySub': '252 questions · find your rank',
    ...
    'g.fanqie.t': 'Fanqie Code Room', 'g.fanqie.d': 'Qi Jiguang\'s two-character signals from the Record of Military Practice: initial from one character, final from the other, 16x16 codebook lookup.',
    'g.otp-telegraph.t': 'OTP Telegraph Room', 'g.otp-telegraph.d': 'Vernam 1917 for real: ITA2 5-unit tape XORed with a key tape — then subtract two same-key messages and watch the depth.',
    'g.tor-onion.t': 'Tor Onion Routing', 'g.tor-onion.d': 'Build a real three-hop circuit under the actual rules: layer and peel the wrap, judge exit visibility, match flows — every skin of the anonymity stack.',
    'g.block-modes.t': 'Block Mode Lab', 'g.block-modes.d': 'The same block cipher, the mode decides: ECB leaks patterns, CBC chains on an IV, CTR streams, GCM delivers both.',
    'g.dh-handshake.t': 'DH Handshake', 'g.dh-handshake.d': 'The 1976 New Directions, computed live: modular exponentiation, shared-secret verification, Eve the MITM, and the degenerate-parameter trap (g=1 makes the key 1).',
    'g.gm-gateway.t': 'GM Gateway', 'g.gm-gateway.d': 'One message through the whole SM pipeline: real SM4 engine encryption, real SM3 digests, SM2-equation verification on a tiny curve, SM9 identity crypto.',
    'g.millionaire.t': 'Millionaires\u2019 Protocol', 'g.millionaire.d': 'Yao\u2019s 1982 founding problem: compare who is richer without revealing the numbers — toy-RSA blinding, 1-out-of-2 OT, first-difference decision tree, the MPC timeline.',
    'g.blind-sign.t': 'Blind Signature', 'g.blind-sign.d': 'Chaum\u2019s 1983 e-cash math for real: RSA blinding, a bank signs a file it never saw, unblind to an ordinary signature, blindness argument and anti-double-spend.',
