...
/* Find the last "chapters: ['quantum'] }," and insert after it */
const marker = "chapters: ['quantum'] },";
const lastIdx = t.lastIndexOf(marker);
if (lastIdx < 0) { console.error('MISS quantum marker'); process.exit(1); }
const insertPos = lastIdx + marker.length;
const NEW = `
...
t = t.slice(0, insertPos) + NEW + t.slice(insertPos);
fs.writeFileSync(H, t);
const count = (t.match(/term:/g) || []).length;
console.log('OK, terms now:', count);
